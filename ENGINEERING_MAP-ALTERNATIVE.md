...
3. **Fraud Proofs** - Permissionless dispute-resolution system ([`dave`](https://github.com/cartesi/dave)). PRT is the algorithm in use today; the eponymous Dave algorithm is research / next. The [Dave Rollups Node](https://github.com/cartesi/dave/tree/main/cartesi-rollups/node) follows application inputs, reproduces execution, and defends correct results in disputes. Coupled tightly to rollups-contracts and the machine step.
...
5. **Sequencer** - Optional SDK component under active development ([`sequencer`](https://github.com/cartesi/sequencer)): provides low-latency soft confirmations and posts user operations in batches to InputBox.
...
| [sequencer](https://github.com/cartesi/sequencer) | Optional centralized sequencer for the SDK: optimistic soft confirmations, deterministic ordering, batched L1 submission, recovery, and watchdog. | Active development |
